...
`docker compose up --build` starts PostgreSQL, API, worker, **Ollama** (`llama3.2` pulled on first start), Prometheus, Grafana, the React dashboard, nginx 500 (`demo-unhealthy`), crash-loop Alpine, and `demo-chaos`.
...
`payments-api` already targets `demo-unhealthy`. Wait for Unhealthy and an open incident. Click **Run analysis** — the API sends stored evidence to Ollama (`llama3.2`) and saves validated JSON. The model cannot run remediations.
